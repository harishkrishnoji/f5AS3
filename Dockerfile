...
# Dependencies
#
FROM git-enterprise-jc.onefiserv.net:4443/public-containers/rundeck/python:3.8.7 AS base
...
ARG PYPI_USERNAME
ARG PYPI_PASSWORD
ENV POETRY_HTTP_BASIC_GITLAB_USERNAME=${PYPI_USERNAME}
...
# TODO(TBD): Enable in compose instead, probably
...
FROM git-enterprise-jc.onefiserv.net:4443/public-containers/rundeck/python:3.8.7-slim AS cli
...
# TODO(TBD): Call out the individual parts of the repo that we need, after repackaging
COPY . .
